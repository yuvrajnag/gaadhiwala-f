...
// import { supabase } from '../supabaseClient'; removed from usage
...
const DriverDashboard = () => {
    ...
    useEffect(() => {
        const mockRides = [
            { id: 'ride-1', pickup_name: 'Airport', drop_name: 'City Center', fare: '₹450', distance: '12 km', status: 'searching' },
            { id: 'ride-2', pickup_name: 'Station', drop_name: 'Hotel Taj', fare: '₹150', distance: '4 km', status: 'searching' }
        ];
        setAvailableRides(mockRides);

        const timer = setTimeout(() => {
            setAvailableRides(prev => [
                { id: 'ride-3-' + Date.now(), pickup_name: 'Tech Park', drop_name: 'Mall', fare: '₹200', distance: '6 km', status: 'searching' },
                ...prev
            ]);
        }, 12000);
    ...
    const handleAcceptRide = async (rideId) => {
        ...
        if (!userStr) return alert('Driver info missing. Please sign in.');
        ...
        if (userStr) {
            ...
            const fetchFreshProfile = () => {
                ...
                const profileData = {
                    ...
                    vehicleType: data.vehicleType || 'Bike/Scooty',
                    experience: data.experience || ''
                };
                setProfile(profileData);
                
                // If profile is missing driver-specific fields, show the form
                const isNewDriver = !data.vehicleModel || !data.vehiclePlate || !data.experience;
